...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            saved_message = await self.save_message(message, sender_id, is_staff, sender_name_payload)
    ...
    def save_message(self, message, sender_id, is_staff, sender_name_payload):
        ...
        msg = ChatMessage.objects.create(
            ...
            sender=sender,
            guest_name=sender_name_payload if not sender else None, # Lưu tên khách nếu có
            message=message,
        ...
        if sender:
            ...
            display_name = sender_name_payload if sender_name_payload else "Khách vãng lai"

        return {
            'message': msg.message,
            'sender_name': display_name,
            'is_staff_reply': msg.is_staff_reply,
            'created_at': timezone.localtime(msg.created_at).strftime('%H:%M'), # Đảm bảo đúng múi giờ
            'avatar': avatar_url,
            'attachment_url': None, 
